...
def bayes_error_plot (ll_ratios, labels, title):
    ...
    for i in range(10):
        actDCF.append(dcf.compute_actual_DCF(eff_plo[i], 1, 1, ll_ratios, labels, True))
        minDCF.append(dcf.compute_min_DCF(eff_plo[i], 1, 1, ll_ratios, labels))
...
def class_calibration (DTR, LTR, DTE, LTE, pi_value, lambda_value):
    ...
    J = disc.lr_obj_wrap(DTR, LTR, lambda_value, pi_value)
    grad = disc.lr_compute_gradient(DTR, LTR, lambda_value, pi_value)
...
def calibrate_scores (scores, labels, model_title):
    ...
    actualDCF = dcf.compute_actual_DCF(pi_t, Cfn, Cfp, numpy.hstack(calibrated), numpy.hstack(lab), False)

    bayes_error_plot(scores, labels, model_title+'_uncalibrated')
    bayes_error_plot(numpy.hstack(calibrated), numpy.hstack(lab), model_title+'_calibrated')

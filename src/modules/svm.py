...
def svm_kfold (D, L, K, pca_value, z_value, C_value, gamma_value):
    ...
    if pca_value==0:
        if z_value==0:
            output_file.write("No PCA, No ZNorm, C: %.5f, gamma: %.5f\n" % (C_value, gamma_value))
            print("No PCA, No ZNorm, C: %.5f, gamma: %.5f\n" % (C_value, gamma_value))
        else:
            output_file.write("No PCA, ZNorm, C: %.5f, gamma: %.5f\n" % (C_value, gamma_value))
            print("No PCA, ZNorm, C: %.5f, gamma: %.5f\n" % (C_value, gamma_value))
    else:
        if z_value==0:
            output_file.write("PCA: %d, No ZNorm, C: %.5f, gamma: %.5f\n" % (pca_value, C_value, gamma_value))
            print("PCA: %d, C: %.5f, No ZNorm, gamma: %.5f\n" % (pca_value, C_value, gamma_value))
        else:
            output_file.write("PCA: %d, ZNorm, C: %.5f, gamma: %.5f\n" % (pca_value, C_value, gamma_value))
